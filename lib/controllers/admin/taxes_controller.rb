...
class Admin::TaxesController < AdminController
  ...
  delete '/admin/taxes/:id' do
    ...
    if orders_count > 0
      flash[:error] = "Cannot delete tax '#{@tax.name}' because it has been used in #{orders_count} order(s). You can deactivate it instead."
      redirect '/admin/taxes'
    else
      tax_name = @tax.name
      @tax.delete
  ...
  post '/api/admin/taxes/preview' do
    ...
    begin
      ...
      if subtotal <= 0
        return { success: false, error: 'Invalid subtotal amount' }.to_json
      end
